...
public class PaissaImporter
{
    ...
    private string ID;
    private string folderText = "尚無資料夾……";
    ...
    public static void DrawPaissaBook(PaissaAddressBookFolder book)
    {
        ...
        else if(ImGui.BeginTable($"##addressbook", 7, ImGuiTableFlags.Borders | ImGuiTableFlags.RowBg | ImGuiTableFlags.SizingFixedFit))
        {
            ...
            ImGui.TableSetupColumn("尺寸");
            ImGui.TableSetupColumn("抽選人數");
            ImGui.TableSetupColumn("購買資格");
            ImGui.TableSetupColumn("世界");
            ImGui.TableSetupColumn("區");
            ImGui.TableSetupColumn("土地");
            ...
            for(var i = 0; i < entryArray.Count; i++)
            {
                ...
                if(ImGui.BeginPopup($"ABMenu {entry.GUID}"))
                {
                    ...
                    if(ImGui.MenuItem("刪除"))
                    {
                        ...
                        {
                            Svc.Toasts.ShowError("按住 CTRL 再點擊即可刪除項目");
                        }
                    }
                    ImGuiEx.Tooltip("按住 CTRL 再點擊即可刪除");
                    ImGui.EndPopup();
                }
